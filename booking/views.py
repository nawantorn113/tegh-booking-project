#
# booking/views.py (ฉบับเต็ม - อัปเกรด search_view)
#
import json
import re 
...
try:
    ...
    print("\n!!! WARNING: dal_select2 not installed !!!\n")
    DAL_AVAILABLE = False
    class Select2QuerySetView: # Placeholder class
        pass
# --- --------------------- ---
...
try: from openpyxl import Workbook; OPENPYXL_AVAILABLE = True
except ImportError: print("\n!!! WARNING: openpyxl not installed !!!\n"); OPENPYXL_AVAILABLE = False
from django.utils import timezone
try: from weasyprint import HTML; WEASYPRINT_AVAILABLE = True
...
def user_logged_in_callback(sender, request, user, **kwargs):
    ip = request.META.get('REMOTE_ADDR'); LoginHistory.objects.create(user=user, action='LOGIN', ip_address=ip)
@receiver(user_logged_out)
def user_logged_out_callback(sender, request, user, **kwargs):
    if user and hasattr(user, 'is_authenticated') and user.is_authenticated:
        ip = request.META.get('REMOTE_ADDR'); LoginHistory.objects.create(user=user, action='LOGOUT', ip_address=ip)

def is_admin(user): return user.is_authenticated and (user.is_superuser or user.groups.filter(name='Admin').exists())
def is_approver_or_admin(user): return user.is_authenticated and (user.is_superuser or user.groups.filter(name__in=['Approver', 'Admin']).exists())
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): user = form.get_user(); login(request, user); next_url = request.GET.get('next', 'dashboard'); messages.success(request, f'ยินดีต้อนรับ, {user.get_full_name() or user.username}!'); return redirect(next_url)
        else: messages.error(request, 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง')
    else: form = AuthenticationForm()
    return render(request, 'login.html', {'form': form})
@login_required
def logout_view(request):
    user_before = request.user; user_pk_before = user_before.pk; logout(request)
    try: user_obj = User.objects.get(pk=user_pk_before); user_logged_out.send(sender=user_obj.__class__, request=request, user=user_obj)
    except Exception as e: print(f"Error sending logout signal for PK {user_pk_before}: {e}")
    messages.success(request, 'ออกจากระบบเรียบร้อยแล้ว'); return redirect('login')
@login_required
def change_password_view(request):
    if request.method == 'POST':
        form = CustomPasswordChangeForm(request.user, request.POST)
        if form.is_valid(): user = form.save(); update_session_auth_hash(request, user); messages.success(request, 'เปลี่ยนรหัสผ่านเรียบร้อยแล้ว!'); return redirect('change_password')
        else: messages.error(request, 'กรุณาแก้ไขข้อผิดพลาดด้านล่าง')
    else: form = CustomPasswordChangeForm(request.user)
    return render(request, 'pages/change_password.html', {'form': form})

@login_required
def dashboard_view(request):
    now = timezone.now(); sort_by = request.GET.get('sort', 'floor'); all_rooms_query = Room.objects.all()
    if sort_by == 'status':
        all_rooms_list = list(all_rooms_query); [setattr(r, 'is_booked', r.bookings.filter(start_time__lte=now, end_time__gt=now, status='APPROVED').exists()) for r in all_rooms_list]; all_rooms_sorted = sorted(all_rooms_list, key=lambda r: r.is_booked)
    ...
    for room in all_rooms_sorted:
        ...
        room.status = 'ไม่ว่าง' if current else 'ว่าง'; room.current_booking_info = current; room.next_booking_info = None
    ...
    my_bookings_today = Booking.objects.filter(booked_by=request.user, start_time__date=now.date()).count()
    summary = { 'total_rooms': Room.objects.count(), 'today_bookings': Booking.objects.filter(start_time__date=now.date(), status='APPROVED').count(), 'pending_approvals': Booking.objects.filter(status='PENDING').count(), 'my_bookings_today': my_bookings_today, }
    form = BookingForm()
    context = {'buildings': dict(buildings), 'summary_cards': summary, 'current_sort': sort_by, 'form': form}
    return render(request, 'pages/dashboard.html', context)


def search_view(request):
    """
    View สำหรับหน้าผลการค้นหา (ฉบับปรับปรุง)
    """
    query = request.GET.get('q', '') 
    
    # เริ่มต้นด้วย QuerySet ว่าง
    results = Room.objects.none() 
    
    if query:
        # 1. ตรวจสอบว่าผู้ใช้พิมพ์แค่ตัวเลขหรือไม่ (เช่น "25")
        #    (เราจะลบคำว่า "คน" หรือ "ที่นั่ง" ออกก่อน)
        capacity_match = re.search(r'^\d+', query) # หาตัวเลขที่ขึ้นต้นประโยค
        
        if capacity_match:
            try:
                # ถ้าเจอตัวเลข (เช่น "25")
                capacity = int(capacity_match.group(0))
                # ให้ค้นหาห้องที่มีความจุ (capacity) มากกว่าหรือเท่ากับ
                results = Room.objects.filter(
                    capacity__gte=capacity
                ).order_by('capacity')
            except ValueError:
                results = Room.objects.none() # ถ้าแปลงเป็น int ไม่ได้
        
        # 2. ถ้าค้นหาด้วยความจุไม่เจอ หรือผู้ใช้พิมพ์เป็นข้อความปกติ
        if not results.exists():
            # ให้ค้นหาในหลายๆ Field พร้อมกัน
            results = Room.objects.filter(
                Q(name__icontains=query) |
                Q(building__icontains=query) |
                Q(floor__icontains=query) |
                Q(location__icontains=query) |
                Q(equipment_in_room__icontains=query)
    ...
    # ชี้ไปที่ template ที่ถูกต้อง (ตามที่เราแก้ครั้งล่าสุด)
...
# --- [สำคัญ] ฟังก์ชัน admin_dashboard_view ที่หายไป ---
@login_required
@user_passes_test(is_admin)
def admin_dashboard_view(request):
    thirty_days_ago = timezone.now() - timedelta(days=30); recent = Booking.objects.filter(start_time__gte=thirty_days_ago, status='APPROVED')
    room_usage = Room.objects.annotate(count=Count('bookings', filter=Q(bookings__in=recent))).order_by('-count')
    dept_q = recent.exclude(Q(booked_by__profile__department__exact='')|Q(booked_by__profile__department__isnull=True)).values('booked_by__profile__department').annotate(count=Count('id')).order_by('-count')
    dept_labels = [d['booked_by__profile__department'] for d in dept_q[:10]]; dept_data = [d['count'] for d in dept_q[:10]]
    room_labels = [r.name for r in room_usage[:10]]; room_data = [r.count for r in room_usage[:10]]
    context = { 'pending_count': Booking.objects.filter(status='PENDING').count(), 'today_bookings_count': Booking.objects.filter(start_time__date=timezone.now().date(), status='APPROVED').count(), 'total_users_count': User.objects.count(), 'total_rooms_count': Room.objects.count(), 'login_history': LoginHistory.objects.select_related('user').order_by('-timestamp')[:7], 'room_usage_labels': json.dumps(room_labels), 'room_usage_data': json.dumps(room_data), 'dept_usage_labels': json.dumps(dept_labels), 'dept_usage_data': json.dumps(dept_data), }
    return render(request, 'pages/admin_dashboard.html', context)
# --- ----------------------------------------- ---

@login_required
def rooms_api(request):
    rooms = Room.objects.all().order_by('building', 'name'); resources = [{'id': r.id, 'title': r.name, 'building': r.building or ""} for r in rooms]; return JsonResponse(resources, safe=False)
@login_required
def bookings_api(request):
    ...
    bookings_query = Booking.objects.filter(start_time__lt=end_dt, end_time__gt=start_dt).exclude(status__in=['REJECTED', 'CANCELLED']).select_related('room', 'booked_by')
    if room_id:
...
def update_booking_time_api(request):
    try:
        data = json.loads(request.body); booking_id = data.get('booking_id'); start_str = data.get('start_time'); end_str = data.get('end_time')
        ...
        booking.start_time = new_start; booking.end_time = new_end; status_message = "Updated."
        if booking.participant_count >= 15 and booking.status not in ['PENDING', 'REJECTED', 'CANCELLED']: booking.status = 'PENDING'; status_message = "Updated. Approval required."
        booking.save(); return JsonResponse({'status': 'success', 'message': status_message, 'new_status': booking.status})
    except json.JSONDecodeError: return JsonResponse({'status': 'error', 'message': 'Invalid JSON.'}, status=400)
    except Booking.DoesNotExist: return JsonResponse({'status': 'error', 'message': 'Not found.'}, status=404)
...
def delete_booking_api(request, booking_id):
    try:
        ...
        if booking.status in ['CANCELLED', 'REJECTED']: return JsonResponse({'success': False, 'error': 'ถูกยกเลิก/ปฏิเสธไปแล้ว'})
        booking.status = 'CANCELLED'; booking.save(); return JsonResponse({'success': True, 'message': 'ยกเลิกเรียบร้อย'})
...
def create_booking_view(request, room_id):
    ...
    if referer_url and f'/room/{room.id}/calendar/' in referer_url: redirect_url = 'room_calendar'; redirect_args = {'room_id': room.id}
    else: redirect_url = 'dashboard'; redirect_args = {}
    if form.is_valid():
        booking = form.save(commit=False); booking.room = room; booking.booked_by = request.user
        count = form.cleaned_data.get('participant_count', 1)
        if count >= 15: booking.status = 'PENDING'; messages.success(request, f"จอง '{booking.title}' เรียบร้อย **รออนุมัติ**")
        else: booking.status = 'APPROVED'; messages.success(request, f"จอง '{booking.title}' **อนุมัติอัตโนมัติ** เรียบร้อย")
        booking.save(); form.save_m2m()
        return redirect(redirect_url, **redirect_args)
    else:
        for field, errors in form.errors.items():
            field_label = form.fields[field].label if field in form.fields and field != '__all__' else 'Error'
...
def edit_booking_view(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            updated = form.save(commit=False); count = form.cleaned_data.get('participant_count', 1)
            changed = any(f in form.changed_data for f in ['start_time', 'end_time', 'participant_count'])
            if count >= 15 and changed and updated.status not in ['PENDING', 'REJECTED', 'CANCELLED']: updated.status = 'PENDING'; messages.info(request, "การแก้ไขต้องรออนุมัติใหม่")
            updated.save(); form.save_m2m(); messages.success(request, "แก้ไขการจองเรียบร้อย"); return redirect('history')
        else: messages.error(request, "ไม่สามารถบันทึกการแก้ไขได้")
...
def delete_booking_view(request, booking_id):
    ...
    booking.status = 'CANCELLED'; booking.save(); messages.success(request, f"ยกเลิก '{booking.title}' เรียบร้อย"); return redirect('history')
...
def approve_booking_view(request, booking_id):
    ...
    booking.status = 'APPROVED'; booking.save(); messages.success(request, f"อนุมัติ '{booking.title}' เรียบร้อย")
...
def reject_booking_view(request, booking_id):
    booking = get_object_or_404(Booking.objects.select_related('booked_by'), id=booking_id, status='PENDING')
    booking.status = 'REJECTED'; booking.save(); messages.warning(request, f"ปฏิเสธ '{booking.title}'")
...
def room_management_view(request):
    rooms = Room.objects.all().order_by('building', 'name'); return render(request, 'pages/rooms.html', {'rooms': rooms})
@login_required
@user_passes_test(is_admin)
def add_room_view(request):
    if request.method == 'POST':
        ...
        else: messages.error(request, "ไม่สามารถเพิ่มห้องได้")
    else: form = RoomForm()
...
def export_reports_excel(request):
    ...
    ws.column_dimensions['A'].width = 35; ws.column_dimensions['B'].width = 15; wb.save(response); return response
@login_required
@user_passes_test(is_admin)
def export_reports_pdf(request):
    ...
    recent = Booking.objects.filter(start_time__date__gte=start, start_time__date__lte=today, status='APPROVED')
    if dept: recent = recent.filter(booked_by__profile__department=dept); title += f" (แผนก: {dept})"
    stats = Room.objects.annotate(count=Count('bookings', filter=Q(bookings__in=rect))).filter(count__gt=0).order_by('-count')
    ...
    try:
        ...
        fname = f'report_{period}_{dept or "all"}_{today:%Y%m%d}.pdf'; response['Content-Disposition'] = f'attachment; filename="{fname}"'; return response
